            ...
            <div class="card shadow" id="vueapp">
                ...
                <div class="card-header d-flex">
                    ...
                    <a href="{{route('admin.post.create')}}"
                       class="btn btn-sm btn-primary me-auto">افزودن مقاله</a>
                </div>
                <div class="card-body">
                    <div class="mb-2">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr v-for="(item, index) in models" :key="index">
                            <td>@{{item.id}}</td>
                            <td>
                                <a :href="`/blog/article/${item.sku}`" target="_blank">@{{item.name}}</a>
                            </td>
                            <td>
                                <a :href="`{{$base_url}}/user/${item.user_id}`" target="_blank">@{{item?.user?.name}}</a>
                            </td>
                            <td>@{{item.views.toLocaleString()}}</td>
                            <td class="actions-btns d-inline">
                                <a :href="`{{$base_url}}/post/${item.id}`" target="_blank"
                                ...
                                <a :href="`{{$base_url}}/post/${item.id}/edit`" target="_blank"
        ...
        createApp({
            ...
            methods:{
                async search(){
                    this.error = '';
                    this.message = '';
                    fetch("{{$base_url}}/search-post?search=" + this.search_text, {
                        headers: {
                ...
                async restoreItem(id, index){
                    ...
                    fetch("{{$base_url}}/restore-post/" + id, {
                        headers: {
                            'Content-type': 'application/json',
                            'Accept': 'application/json'
                        },
                        ...
                        if(res.status >=200 && res.status <= 204)
                        {
                            this.message = 'مقاله با موفقیت منتشر شد';
